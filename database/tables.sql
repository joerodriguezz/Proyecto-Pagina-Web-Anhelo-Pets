...
CREATE TABLE rescue_records (
    ...
    CONSTRAINT fk_rescue_records_animal_id
        ...
        ON UPDATE CASCADE
        ON DELETE SET NULL,
    CONSTRAINT fk_rescue_records_foster_home_id
        FOREIGN KEY (foster_home_id)
        REFERENCES foster_homes (foster_home_id)
        ON UPDATE CASCADE
        ON DELETE SET NULL

    -- FK omitida a propósito: volunteer_id almacena el user_id que guarda el
    -- localStorage del frontend, no un volunteers.volunteer_id real.
);
...
CREATE TABLE donations (
    donation_id      bigint GENERATED ALWAYS AS IDENTITY,
    ...
    CONSTRAINT ck_donations_currency CHECK (currency IN ('CRC', 'USD')),
...
CREATE INDEX ix_donations_validation_status
    ON donations (validation_status);
